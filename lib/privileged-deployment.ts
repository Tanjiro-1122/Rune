...
export async function runPrivilegedDeployment(input: PrivilegedDeploymentInput): Promise<PrivilegedDeploymentResult> {
  ...
  try {
    ...
    // The legacy deployment-control module still calls this operation execute_redeploy.
    // This wrapper is the owner-facing gate: it only passes the legacy approval phrase
    // after APPPROVE RUNE DEPLOY / APPROVE RUNE ROLLBACK has already passed the shared
    // privileged-operation policy, dry-run evidence, scope validation, and audit logging.

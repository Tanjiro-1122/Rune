...
const PRIVATE_DEPLOY_APPROVAL_PHRASE = "APPROVE PRIVATE RUNE DEPLOY";
...
export async function createAppCreatorProposal(input: AppCreatorInput): Promise<AppCreatorResult> {
  ...
  const proposalResult = await createRepoActionProposal({
    ...
    findings: [
      `Javier asked Rune to create an app from this idea: ${idea}`,
      "Rune can create this through a controlled App Creator workflow, starting with a blueprint and Repo Control proposal.",
...
export function buildAppScaffoldPatch(plan: AppCreatorPlan) {
  ...
  files.set(`docs/app-creator/${plan.slug}.md`, [
    `# ${plan.appName}`,
    "",
    "Generated by Rune App Creator v1.1 after proposal approval.",
    "",
    `- Platform: ${plan.platform}`,
    `- Complexity: ${plan.complexity}`,
    `- Target users: ${plan.targetUsers}`,
  ...
  files.set(`${base}/README.md`, [
    `# ${plan.appName}`,
    "",
    "Starter app scaffold generated by Rune App Creator v1.1.",
    "",
    "## What is included",
...
export async function queuePrivateAppCreatorDeploy(options: {
  ...
  const queued = await queueCliRunnerJob({
    ...
    metadata: {
      ...
      previewHandoff: handoff.previewHandoff,
      safety: "queued_private_owner_only_executor_artifact_no_public_launch_no_merge_no_schema_mutation",
    },
  });
  ...
  await logActionEvent({
    ...
    workspaceId: options.workspaceId || proposal.workspace_id || null,
    conversationId: options.conversationId || proposal.conversation_id || null,
    metadata: { proposalId, taskId: queued.taskId, commandPreview: command, ownerOnly: true, targetAudience: "javier_only", authRequired: true, executorMode: "owner_only_executor_v1", safety: "queued_private_owner_only_executor_artifact_no_public_launch_no_merge_no_schema_mutation" },
  });
  ...
  return {
    ...
    commandPreview: command,
    requiredApprovalPhrase: PRIVATE_DEPLOY_APPROVAL_PHRASE,
    message: "App Creator v1.7 queued a private owner-only executor job for the trusted runner. Rune did not deploy from chat, merge, mutate schema, or make the app public.",
    safety: "queued_private_owner_only_executor_artifact_no_public_launch_no_merge_no_schema_mutation",
    nextAction: "Watch the Runner dashboard. If the external runner has JARVIS_PRIVATE_OWNER_EXECUTOR_MODE=execute, it will create an artifact-only private deployment manifest; otherwise it will complete dry-run validation only.",
  };
}

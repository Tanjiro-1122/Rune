...
const calendarSkill: SkillDefinition = {
  name: "get_calendar",
  description:
    "Read upcoming Google Calendar events. Requires GOOGLE_CALENDAR_ACCESS_TOKEN env var. " +
    "Use when Javier asks what is on the calendar, upcoming meetings, or schedule.",
  tool: tool({
    description: "Get upcoming Google Calendar events.",
    parameters: z.object({
      days_ahead: z.number().min(1).max(30).optional().default(7).describe("How many days ahead to look"),
    ...
    execute: async ({ days_ahead = 7, max_events = 10 }) => {
      try {
        ...
        if (!token) return { error: "GOOGLE_CALENDAR_ACCESS_TOKEN not configured. Connect Google Calendar in Rune settings." };

...
const ownerPreviewTokenRoute = fs.readFileSync('scripts/prepare-owner-preview-token-route.mjs', 'utf8');
...
const checks = [
  ...
  ['checkpoint UI exists', /latest_checkpoint|checkpoint|Latest checkpoint/i.test(chatUi + css)],
  ...
  ['external runner spec exists', /External Runner Spec v1/.test(externalRunnerSpec) && /RUNE_RUNNER_TOKEN/.test(externalRunnerSpec) && /APPROVE RUNE ROLLBACK/.test(externalRunnerSpec) && /must not execute/.test(externalRunnerSpec)],
  ['private app creator runner spec exists', /private_app_creator_deploy/.test(externalRunnerSpec) && /APPROVE PRIVATE RUNE DEPLOY/.test(externalRunnerSpec) && /owner_only_authenticated_javier/.test(externalRunnerSpec) && /owner-only artifact executor/.test(externalRunnerSpec) && /Private access verification v1.8/.test(externalRunnerSpec) && /Owner-only hosted preview gate v1.9/.test(externalRunnerSpec) && /Protected owner preview execution v1.10/.test(externalRunnerSpec) && /Protected hosting provider adapter v1.11/.test(externalRunnerSpec) && /Signed owner preview token contract v1.12/.test(externalRunnerSpec) && /Owner preview token route contract v1.13/.test(externalRunnerSpec)],
  ...
  ['owner preview token route contract exists', /prepare-owner-preview-token-route/.test(packageJson) && /owner_preview_token_route_contract_prepared_no_route_no_token_no_url/.test(ownerPreviewTokenRoute) && /owner-preview-token-routes/.test(ownerPreviewTokenRoute) && /APPROVE OWNER PREVIEW TOKEN ROUTE IMPLEMENTATION/.test(ownerPreviewTokenRoute)],
  ['owner preview token route contract blocks implementation and urls', !/--prod/.test(ownerPreviewTokenRoute) && !/vercel deploy/.test(ownerPreviewTokenRoute) && /apiRouteFileCreated: false/.test(ownerPreviewTokenRoute) && /tokenGenerated: false/.test(ownerPreviewTokenRoute) && /tokenRouteCreated: false/.test(ownerPreviewTokenRoute) && /publicPreviewUrl: null/.test(ownerPreviewTokenRoute) && /protectedPreviewUrl: null/.test(ownerPreviewTokenRoute)],
  ['runner dashboard visibility exists', /runner|job|Private App Creator deploy/i.test(chatUi + css)],
  ...
  ['external services UI exists', /external|service/i.test(chatUi + css)],
  ['revenuecat lookup UI exists', /RevenueCat|revenuecat/i.test(chatUi + css)],
  ['app store connect lookup UI exists', /App Store Connect|app_store/i.test(chatUi + css)],
  ['google play lookup UI exists', /Google Play|google_play/i.test(chatUi + css)],
  ['app health snapshot UI exists', /app_health|Health|health snapshot/i.test(chatUi + css)],
  ['repo control flow UI exists', /repo_control|Repo Control/i.test(chatUi + css)],
  ['deployment handoff UI exists', /deployment|handoff/i.test(chatUi + css)],
  ['operator console UI exists', /Operator|operator/i.test(chatUi + css)],
];

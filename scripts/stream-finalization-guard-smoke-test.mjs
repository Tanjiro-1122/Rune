...
const checks = [
  ['backend persistence timeout constant exists', route.includes('CHAT_FINISH_PERSISTENCE_TIMEOUT_MS = 8_000')],
  ['backend wraps onFinish persistence with timeout', route.includes('withChatFinishTimeout') && route.includes('chat finish persistence and task completion')],
  ...
  ['frontend stale stream recovery timeout exists', chat.includes('STREAM_FINALIZATION_RECOVERY_MS = 12_000')],
  ...
  ['existing useChat path remains intact', chat.includes('useChat({') && chat.includes('handleSubmit(e, {') && route.includes('return result.toDataStreamResponse();')],
];

...
const checks = [
  ...
  ['agent work loop module exists', fs.existsSync('lib/agent-work-loop.ts') && /Agent Core Work Loop/.test(fs.readFileSync('lib/agent-work-loop.ts', 'utf8'))],
  ...
  ['deployment handoff exists', /prepareRepoDeploymentHandoff/.test(repoActions) && /metadata_only_no_deploy/.test(repoActions) && /APPROVE RUNE REDEPLOY/.test(repoActions)],
  ...
  ['deployment executor gate exists', /executeDeploymentControlAction/.test(deploymentControl) && /APPROVE RUNE REDEPLOY/.test(deploymentControl) && /cli_runner_not_enabled/.test(deploymentControl) && /execute_redeploy/.test(route)],
  ['specific tool labels exist', /getToolDisplayLabel/.test(chatUi) && /Preparing rollback approval/.test(chatUi)],
  ['repo/deployment UI status details exist', /readinessSummary/.test(chatUi) && /Required phrase/.test(chatUi) && /repo-control-status/.test([fs.readFileSync('app/globals.css', 'utf8'), fs.readFileSync('app/chat-mobile.css', 'utf8'), fs.readFileSync('app/operator.css', 'utf8')].join('\n'))],
  ...
  ['Rune voice layer exists', /private AI operator/.test(chatRoute) && /Talk like a real person/.test(chatRoute) && /consultant/.test(chatRoute) && /safety/i.test(chatRoute)],
  ...
  ['app store connect chat tool exists', /lookup_app_store_connect_status/.test(route) && /getAppStoreConnectReadOnlySummary/.test(route) && /read-only/i.test(route)],
  ...
  ['google play chat tool exists', /lookup_google_play_status/.test(route) && /getGooglePlayReadOnlySummary/.test(route) && /read-only/i.test(route)],
...
const diagnosticChecks = [
  ...
  ['operator diagnostic regex uses safe word boundaries', chatRoute.includes('operator console') && chatRoute.includes('today') && !chatRoute.includes('\u0008')],

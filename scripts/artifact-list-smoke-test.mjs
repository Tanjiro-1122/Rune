...
assert(drawer.includes("<ArtifactList artifacts={artifacts}"), "artifact drawer renders extracted artifact list");
assert(!chat.includes("artifacts.map((artifact) =>"), "chat no longer maps artifact list inline");
assert(chat.includes("<ArtifactDrawerSection artifacts={artifacts}"), "chat renders artifact drawer that owns the artifact list");

...
const PROJECTS = [
  ...
  { key: "unfiltr", label: "Unfiltr", repo: "Tanjiro-1122/UniltrbyJavierbackup", tag: "SENSITIVE PRODUCTION", color: "#e67e22" },
    ...
    <div style={{ display:"grid", gridTemplateColumns:"48px 200px 1fr", gridTemplateRows:"44px 1fr", height:"100vh", background:"#0a0a0a", fontFamily:"'JetBrains Mono','Fira Code',monospace", color:"#d4d4d4", overflow:"hidden" }}>
      ...
      <div style={{ gridColumn:"1 / -1", background:"#080808", borderBottom:"1px solid #1a1a1a", display:"flex", alignItems:"center", padding:"0 16px", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:6, marginLeft:"auto" }}>
          {PROJECTS.map(p => (
            <button key={p.key} onClick={() => setActiveProject(p.key)} style={{ fontSize:10, padding:"3px 10px", borderRadius:20, border: activeProject===p.key ? `1px solid ${p.color}` : "1px solid #222", background: activeProject===p.key ? p.color+"22" : "transparent", color: activeProject===p.key ? p.color : "#555", cursor:"pointer", fontFamily:"inherit" }}>{p.label}</button>
          ))}
        </div>
      ...
      <div style={{ gridRow:2, background:"#080808", borderRight:"1px solid #141414", display:"flex", flexDirection:"column", alignItems:"center", padding:"10px 0", gap:2 }}>
        {NAV.map((n,i) => (
          <div key={n.id}>
            ...
            <button onClick={() => setActiveNav(n.id)} title={n.label} style={{ width:34, height:34, borderRadius:7, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", border:"none", background: activeNav===n.id ? "#1e0a0a" : "transparent", color: activeNav===n.id ? "#c0392b" : "#3a3a3a", fontSize:16, fontFamily:"inherit" }}>{n.icon}</button>
          </div>
        ))}
        <button style={{ marginTop:"auto", width:34, height:34, borderRadius:7, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", border:"none", background:"transparent", color:"#2a2a2a", fontSize:16, fontFamily:"inherit" }}>⚙</button>
      </div>

      <div style={{ gridRow:2, background:"#0b0b0b", borderRight:"1px solid #141414", overflowY:"auto", padding:"14px 0" }}>
        {[
          { label:"Repo control", items:[{icon:"⎇",text:"Open PRs",badge:"3",bc:"#c0392b"},{icon:"○",text:"Pending approval",badge:"1",bc:"#e67e22"},{icon:"✓",text:"Executed",badge:"12",bc:"#333"}]},
          { label:"Projects", items:PROJECTS.map(p=>({icon:"◉",text:p.label,badge:"ok",bc:"#1e6b3a"}))},
          { label:"Quick actions", items:[{icon:"+",text:"Create file",badge:null,bc:""},{icon:"✎",text:"Edit file",badge:null,bc:""},{icon:"↑",text:"Deploy",badge:null,bc:""}]},
        ].map(section => (
          <div key={section.label} style={{ padding:"0 10px", marginBottom:20 }}>
            ...
            {section.items.map(item => (
              <div key={item.text} style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 8px", borderRadius:5, cursor:"pointer", fontSize:11, color:"#555" }}>
      ...
      <div style={{ gridRow:2, display:"flex", flexDirection:"column", background:"#0e0e0e", overflow:"hidden" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, padding:"14px 20px", borderBottom:"1px solid #141414" }}>
          {[{label:"Open PRs",value:"3",color:"#e8e8e8"},{label:"Last deploy",value:"✓ live",color:"#27ae60"},{label:"Pending approval",value:"1",color:"#f59e0b"},{label:"Token expiry",value:"2d",color:"#c0392b"}].map(s => (
        ...
        <div style={{ borderTop:"1px solid #141414", padding:"10px 16px", display:"flex", alignItems:"center", gap:10, background:"#090909" }}>
          <input value={input} onChange={e => setInput(e.target.value)} placeholder="Ask Rune anything..." style={{ flex:1, background:"#111", border:"1px solid #1e1e1e", borderRadius:6, padding:"7px 12px", fontSize:11, color:"#d4d4d4", outline:"none", fontFamily:"inherit" }} />

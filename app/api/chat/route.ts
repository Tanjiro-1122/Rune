import { streamText, UIMessage, convertToCoreMessages, tool }
        ...
        proposeHandsAction: {
          description:
            "Propose a sensitive action (code change, deploy, PR, merge, schema change, customer message, financial action). " +
            "ALWAYS call this before executing any sensitive action. Returns a proposal with a gate phrase Javier must send to approve. " +
            "Never skip this for: code changes, deployments, merges, schema mutations, customer messages, credits, financial actions.",
          parameters: z.object({
            actionType: z
              .enum(["code_change","deploy","pr_open","pr_merge","branch_create","schema_change","customer_message","grant_credits","financial","revoke_access","other_sensitive"])
              .describe("Category of sensitive action"),
            title: z.string().describe("Short title, e.g. 'Fix typo in ChatPage.tsx'"),
            findings: z.string().describe("What you found — the facts that justify this action"),
            plan: z.string().describe("Step-by-step plan for what will be done"),
            riskLevel: z.enum(["low","medium","high"]).optional().describe("Risk level — default medium"),
            projectKey: z.string().optional().describe("Project this action targets, e.g. 'rune', 'unfiltr', 'swh'"),
            rollbackNote: z.string().optional().describe("How to undo this if it goes wrong"),
          }),
          execute: async ({ actionType, title, findings, plan, riskLevel, projectKey, rollbackNote }) => {
            ...
            return {
              ...
              gatePhrase: result.proposal!.gate_phrase,
              status: "proposed",
              message: `Proposal created. Javier must send: ${result.proposal!.gate_phrase}`,
            };
          },
        },
        ...
        approveHandsAction: {
          description:
            "Approve a pending Hands proposal after Javier sends the gate phrase. " +
            "Call this when Javier's message matches or contains the gate phrase from a proposeHandsAction result.",
          parameters: z.object({
            proposalId: z.string().describe("The proposal ID from proposeHandsAction"),
          }),
          execute: async ({ proposalId }) => {
            ...
            return {
              proposalId,
              status: "approved",
              actionType: result.proposal!.action_type,
        ...
        listHandsProposalsAction: {
          description: "List recent Hands proposals and their status (proposed, approved, executed, failed).",
          parameters: z.object({

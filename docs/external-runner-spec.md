...
runner_metadata.job_kind = vercel_redeploy | vercel_rollback | repo_check | maintenance
...
5. The command is one of the documented Vercel CLI commands Jarvis prepared:
   - `vercel redeploy <deployment-url-or-id> --token=$VERCEL_TOKEN`
   - `vercel rollback <deployment-url-or-id> --token=$VERCEL_TOKEN`

If any check fails, the runner must call `/api/runner` with `action: "fail"` and must not execute the command.
